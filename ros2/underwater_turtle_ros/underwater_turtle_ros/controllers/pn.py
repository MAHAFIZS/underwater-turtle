...
class PNParams:
    ...
    # === Near-goal caps (reduces circling) ===
...
class PNController:
    ...
    Proportional Navigation controller + stabilizers:
      ...
      - v slew-rate limiting
      - near-goal speed caps

    Returns (omega_cmd, v_cmd).
    """
    ...
    def compute(
        ...
        *,
        ...
        v_max: float,
        dt: float,
        # Optional extras for gating (can be zeros if unknown)
        fish_vx: float = 0.0,
        fish_vy: float = 0.0,
        ...
        **_ignored,
        ...
        # LOS rate (wrapped) + EMA
        los_rate = wrap_angle(los - self._los_prev) / dt
        self._los_prev = los

        a = clamp(float(self.p.pn_los_rate_alpha), 0.0, 0.99)
        ...
        # PN omega command
        omega_cmd = float(self.p.pn_k_los) * err + (float(self.p.pn_N) * gate) * los_rate_use
        ...
        # omega hard cap
        omega_cmd = clamp(omega_cmd, -float(self.p.omega_abs_max), float(self.p.omega_abs_max))
        self._omega_cmd_prev = omega_cmd
        ...
        v_nom = float(self.p.v_base) + float(self.p.v_gain) * dist
